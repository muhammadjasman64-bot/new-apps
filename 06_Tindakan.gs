...
function updateTindakanStatus_(id,status,keterangan){
  ...
  sh.getRange(row,1,1,vals.length).setValues([vals]);clearAppCache_();PropertiesService.getScriptProperties().setProperty('DB_VERSION',String(Date.now()));if(typeof syncNisnDatabase_==='function')syncNisnDatabase_();logActivity_('UBAH STATUS TINDAKAN',String(id));return {success:true};
}
...
function updatePemanggilanStatus_(id,status,hasil,tindakLanjut){
  ...
  const row=i+2,h=d.map,vals=sh.getRange(row,1,1,sh.getLastColumn()).getValues()[0];const set=(names,v)=>{const c=dbFindColumn_(h,names);if(c>=0)vals[c]=v;};set(['Status'],status||'Selesai');set(['Hasil'],hasil||'');set(['Tindak_Lanjut'],tindakLanjut||'');sh.getRange(row,1,1,vals.length).setValues([vals]);clearAppCache_();PropertiesService.getScriptProperties().setProperty('DB_VERSION',String(Date.now()));if(typeof syncNisnDatabase_==='function')syncNisnDatabase_();logActivity_('UBAH STATUS PEMANGGILAN',String(id));return {success:true};
}
...
function createPdfSuratPemanggilan_(nisn,ke){
  const key=pdfRequestKey_('surat',{nisn:canonicalNisn_(nisn),ke:Number(ke||0)}),cached=cacheGetJson_(key);
