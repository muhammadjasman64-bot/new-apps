...
function createPdfSuratPemanggilan_(nisn,ke){const key=pdfRequestKey_('surat',{nisn:String(nisn||''),ke:Number(ke||0)}),cached=cacheGetJson_(key);if(cached&&cached.id){try{const existing=DriveApp.getFileById(cached.id);if(existing.getMimeType()===MimeType.PDF&&existing.getSize()>0)return cached;}catch(e){}}const html=renderSuratPemanggilanHtml_(nisn,ke),stamp=Utilities.formatDate(new Date(),Session.getScriptTimeZone(),'yyyyMMdd_HHmmss'),blob=Utilities.newBlob(html,'text/html','surat.html').getAs(MimeType.PDF).setName('Surat_Pemanggilan_'+nisn+'_'+stamp+'.pdf');if(blob.getContentType()!=='application/pdf')throw new Error('Gagal membuat surat PDF. File hasil konversi bukan PDF.');const bytes=blob.getBytes();if(!bytes.length)throw new Error('Gagal membuat surat PDF. File PDF kosong.');const f=getOrCreateFolder_().createFile(blob),result=buildPdfResult_(f);cachePutJson_(key,result,300);return result;}
