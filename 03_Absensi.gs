...
function saveAbsensi_(records){
  ...
  const rowByKey={}; all.forEach((r,i)=>{const n=canonicalNisn_(cN>=0?r[cN]:''),d=cT>=0?formatDateKey_(r[cT]):'';if(n&&d)rowByKey[n+'|'+d]=i+2;});
  ...
  records.forEach(x=>{
    ...
    const dt=parseDateInput_(x.tanggal)||now, key=canonicalNisn_(s.nisn)+'|'+formatDateKey_(dt), existingRow=rowByKey[key];
    const vals={};
    if(cId>=0)vals[cId]=existingRow?sh.getRange(existingRow,cId+1).getValue():generateID_('ABS');
    if(cT>=0)vals[cT]=dt;if(cN>=0)vals[cN]=canonicalNisn_(s.nisn);if(cNm>=0)vals[cNm]=s.nama;if(cK>=0)vals[cK]=x.kelas||s.kelas;if(cS>=0)vals[cS]=status;if(cKet>=0)vals[cKet]=x.keterangan||'';if(cU>=0)vals[cU]=user;if(cY>=0)vals[cY]=cfg.Tahun_Pelajaran||getTahunPelajaran_();if(cSem>=0)vals[cSem]=cfg.Semester||getSemesterAktif_();
...
function getRekapAbsensi_(period,year,month,semester,kelas,weekStart){
  ...
  const resultMap={};siswa.forEach(s=>{resultMap[canonicalNisn_(s.nisn).trim()]={H:0,S:0,I:0,A:0,T:0,D:0};});
  ...
  const result=siswa.map(s=>{const z=resultMap[canonicalNisn_(s.nisn).trim()]||{H:0,S:0,I:0,A:0,T:0,D:0},total=z.H+z.S+z.I+z.A+z.T+z.D,had=z.H+z.T;return Object.assign({},s,z,{total:total,kehadiran:total?Number((had/total*100).toFixed(2)):0});});
