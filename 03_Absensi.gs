...
function saveAbsensi_(records){
  ...
  const rowByKey={}; all.forEach((r,i)=>{const n=String(cN>=0?r[cN]:'').trim(),d=cT>=0?formatDateKey_(r[cT]):'';if(n&&d)rowByKey[n+'|'+d]=i+2;});
  const updates=[],adds=[]; let updated=0,inserted=0;
  records.forEach(x=>{
    const s=findStudentByNisn_(x.nisn);if(!s)throw new Error('NISN tidak ditemukan: '+x.nisn);
    ...
    const dt=parseDateInput_(x.tanggal)||now, key=String(s.nisn).trim()+'|'+formatDateKey_(dt), existingRow=rowByKey[key];
    const vals={};
    if(cId>=0)vals[cId]=existingRow?sh.getRange(existingRow,cId+1).getValue():generateID_('ABS');
    if(cT>=0)vals[cT]=dt;if(cN>=0)vals[cN]=String(s.nisn);if(cNm>=0)vals[cNm]=s.nama;if(cK>=0)vals[cK]=x.kelas||s.kelas;if(cS>=0)vals[cS]=status;if(cKet>=0)vals[cKet]=x.keterangan||'';if(cU>=0)vals[cU]=user;if(cY>=0)vals[cY]=cfg.Tahun_Pelajaran||getTahunPelajaran_();if(cSem>=0)vals[cSem]=cfg.Semester||getSemesterAktif_();
...
function getAbsensiByDate_(dateStr,kelas){
  ...
  if(ct<0||cn<0||cs<0)return out;all.slice(1).forEach(r=>{if(formatDateKey_(r[ct])===d&&(!kelas||ck<0||String(r[ck])===String(kelas)))out[String(r[cn])]={status:r[cs],keterangan:cket>=0?r[cket]:''};});return out;
}
function getRekapAbsensi_(period,year,month,semester,kelas,weekStart){
  ...
  const resultMap={};siswa.forEach(s=>{resultMap[String(s.nisn).trim()]={H:0,S:0,I:0,A:0,T:0,D:0};});
  if(lastRow>=2&&lastCol){
    ...
    all.slice(1).forEach(r=>{
      ...
      if(kelasFilter&&ck>=0&&String(r[ck]||'').trim()!==kelasFilter)return;const nisn=String(r[cn]??'').trim(),st=String(r[cs]??'').trim().toUpperCase();if(!resultMap[nisn]||!Object.prototype.hasOwnProperty.call(resultMap[nisn],st))return;resultMap[nisn][st]++;
    });
  }
  const result=siswa.map(s=>{const z=resultMap[String(s.nisn).trim()]||{H:0,S:0,I:0,A:0,T:0,D:0},total=z.H+z.S+z.I+z.A+z.T+z.D,had=z.H+z.T;return Object.assign({},s,z,{total:total,kehadiran:total?Number((had/total*100).toFixed(2)):0});});
...
function getAbsensiTable_(dateStr,kelas){
  ...
  return all.slice(1).filter(r=>formatDateKey_(r[cT])===key&&(!kelas||cK<0||String(r[cK]).trim()===String(kelas).trim())).map(r=>({id:cId>=0?r[cId]:'',tanggal:formatTanggal_(r[cT]),nisn:cN>=0?r[cN]:'',nama:cNm>=0?r[cNm]:'',kelas:cK>=0?r[cK]:'',status:cS>=0?r[cS]:'',keterangan:cKet>=0?r[cKet]:'',user:cU>=0?r[cU]:'',tahunPelajaran:cY>=0?r[cY]:'',semester:cSem>=0?r[cSem]:''}));
}
...
function deleteAbsensi_(tanggal,nisn){
  ...
  const key=formatDateKey_(parseDateInput_(tanggal)); const row=all.findIndex((r,i)=>i>0&&formatDateKey_(r[ct])===key&&String(r[cn]||'').trim()===String(nisn).trim());if(row<1)throw new Error('Data absensi tidak ditemukan.');sh.deleteRow(row+1);clearAppCache_();PropertiesService.getScriptProperties().setProperty('DB_VERSION',String(Date.now()));logActivity_('HAPUS ABSENSI',String(nisn)+' - '+key);return{success:true};
}
